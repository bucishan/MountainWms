...
namespace Services
{
    public class Sys_roleServices : BaseServices<Sys_role>, ISys_roleServices
    {
        ...
        public Sys_roleServices(ISys_menuServices menuServices,
            ISys_rolemenuServices rolemenuServices,
            ISys_rolemenuauthServices rolemenuauthServices,
            SqlSugarClient client,
            ISys_roleRepository repository) : base(repository)
        {
        ...
        public List<RoleAuthDto> GetRoleMenu(long roleId)
        {
            var listAll = _client.Queryable<Sys_rolemenu, Sys_menu>
                ((s, c) => new object[] {
                   JoinType.Left,s.MenuId==c.MenuId,
                 ...
                 .Select((s, c) => new
                 {
                     ...
                     c.MenuId,
                     c.Sort
                 }).MergeTable().Where((s) => s.IsDel == 1 && s.Status == 1 && s.RoleId == roleId.ToString()).OrderBy(s => s.Sort).ToList();
            var listRoleMenu = listAll.Where(s => s.MenuParent != -1).ToList();
            List<RoleAuthDto> modelList = new List<RoleAuthDto>();
            listRoleMenu.ForEach(p =>
            {
                var modelChildren = _client.Queryable<Sys_menuauth, Sys_auth>
                ((s, c) => new object[] {
                   JoinType.Right,s.AuthId==c.AuthId,
                 }).Select((s, c) => new RoleAuthChildrenDto
                 {
                     MenuAuthId = s.MenuAuthId.ToString(),
                     MenuId = s.MenuId.ToString(),
                     AuthId = c.AuthId.ToString(),
                     AuthName = c.AuthName.ToString()
                 }).MergeTable().Where((s) => s.MenuId == p.MenuId.ToString()).ToList();
                RoleAuthDto model = new RoleAuthDto();
                model.RoleId = p.RoleId.ToString();
                model.MenuId = p.MenuId.ToString();
                model.MenuName = p.MenuName.ToString();
                model.Children = modelChildren;
                modelList.Add(model);
            });
            return modelList;
        }
        public DbResult<bool> UpdateAuth(Sys_role role, long userId, string[] menuAuthId)
        {
            var list = _rolemenuauthServices.QueryableToList(c => c.RoleId == role.RoleId);
            var arr = list.Select(c => c.MenuAuthId.ToString()).ToArray();

            //menuId 页面上的菜单Id;
            role.ModifiedBy = userId;
            role.ModifiedDate = DateTimeExt.DateTime;
            //role.RoleType = "#";
            string[] pageId = arr.Union(menuAuthId).Except(menuAuthId).ToArray(); //delete
            string[] dataId = menuAuthId.Union(arr).Except(arr).ToArray();  //insert
            return _repository.UseTran(() =>
            {
                ...
                if (pageId.Any())
                {
                    ...
                    {
                        array.Add(list.Where(c => c.RoleId == role.RoleId && c.MenuAuthId == item.ToInt64()).SingleOrDefault().RoleMenuAuthId);
                    }
                    _rolemenuauthServices.Delete(array.ToArray());
                }
                if (dataId.Any())
                {
                    var roleList = new List<Sys_rolemenuauth>();
                    foreach (var item in dataId)
                    {
                        roleList.Add(new Sys_rolemenuauth
                        {
                            CreateBy = userId,
                            MenuAuthId = item.ToInt64(),
                            RoleId = role.RoleId,
                            RoleMenuAuthId = PubId.SnowflakeId
                        });
                    }
                    _rolemenuauthServices.Insert(roleList);
                }
            });
        }
        ...
        public DbResult<bool> Update(Sys_role role, long userId, string[] menuId)
        {
            ...
            var arr = list.Select(c => c.MenuId.ToString()).ToArray();
